           ...
           if(p1 >= line.size()) {
            ...
            if(isalpha(line[p1])) { 
                ...
                if (tokenMap.find(line.substr(p1, p2 - p1)) != tokenMap.end()) {
                    string token = tokenMap[line.substr(p1, p2 - p1)];
                    lexemeTokenStream.push_back(make_pair(line.substr(p1, p2 - p1), token));
                } else {
                    lexemeTokenStream.push_back(make_pair(line.substr(p1, p2 - p1), "id"));
                }
                p1 = p2;
            }//Number DFA
                ...
                if (line[p2] == '.' && p2 + 1 < line.size() && isdigit(line[p2 + 1])) {
                    ...
                    lexemeTokenStream.push_back(make_pair(line.substr(p1, p2 - p1), "real"));
                }
                else {
                    lexemeTokenStream.push_back(make_pair(line.substr(p1, p2 - p1), "num"));
                }
                p1 = p2;
            }//Comment DFA
                ...
                if(doubleOperator(characterPair)){
                    ...
                    lexemeTokenStream.push_back(make_pair(line.substr(p1, 2), token));
                    p1 += 2;
                }
                else{
                    string token = tokenMap[line.substr(p1, 1)];
                    lexemeTokenStream.push_back(make_pair(line.substr(p1, 1), token));
                    p1++;
                }         
                ...
                if (tokenMap.find(line.substr(p1, 1)) != tokenMap.end()) {
                    ...
                    lexemeTokenStream.push_back(make_pair(line.substr(p1, 1), "Unknown Token"));
                }
                p1++;
            }
