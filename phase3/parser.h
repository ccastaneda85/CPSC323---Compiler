...
SyntaxTree* parser(vector<Token>& tokenStream) {
    initializeParsingTable();

    // Stack for parsing; stores both the grammar symbol and corresponding CST node
    stack<pair<string, SyntaxTreeNode*>> parsingStack;
    int p1 = 0;

    // Create the root of the syntax tree
    SyntaxTree* tree = new SyntaxTree("<Program>");
    SyntaxTreeNode* rootNode = tree->getRoot();

    // Push the end-of-input marker and root production onto the parsing stack
    parsingStack.push({"$", nullptr});
    parsingStack.push({"<Program>", rootNode});

    while (!parsingStack.empty()) {
        string topSymbol = parsingStack.top().first;
        SyntaxTreeNode* parentNode = parsingStack.top().second;
        parsingStack.pop();

        // Handle empty symbols (epsilon productions)
        if (topSymbol.empty()) {
            continue;
        }

        // If the top of the stack matches the current token
        if (topSymbol == tokenStream.at(p1).getType()) {
            // Create a leaf node for the matched token
            SyntaxTreeNode* leafNode = new SyntaxTreeNode(tokenStream.at(p1).getType());
            if (parentNode) {
                parentNode->addChild(leafNode);
            }
            p1++; // Move to the next token
        }
        // If there's a rule in the parsing table
        else if (parsingTable.find({topSymbol, tokenStream.at(p1).getType()}) != parsingTable.end()) {
            // Get the production rule
            int productionIndex = parsingTable[{topSymbol, tokenStream.at(p1).getType()}];
            vector<string> rhs = productions[productionIndex].rhs;

            // Create child nodes for each symbol in the RHS
            vector<SyntaxTreeNode*> childNodes;
            for (const string& symbol : rhs) {
                SyntaxTreeNode* childNode = new SyntaxTreeNode(symbol);
                childNodes.push_back(childNode);
            }

            // Add the child nodes to the current parent node
            if (parentNode) {
            ...
            // Push the RHS symbols onto the stack in reverse order (to maintain left-to-right parsing)
            for (int i = rhs.size() - 1; i >= 0; --i) {
                parsingStack.push({rhs[i], childNodes[i]});
            }
        }
        // If no matching rule is found, report a syntax error
        else {
    ...
    return tree; // Return the constructed syntax tree
}
